...
$query = "SELECT carga.booking, cntr.cntr_number, cntr.cntr_type, carga.custom_place, cntr.status_cntr FROM cntr INNER JOIN carga ON carga.booking = cntr.booking WHERE main_status = 'CUSTOM PLACE' AND `user_cntr` = '$user'";
...
if(isset($_POST["export_data"])) { //  comprobar que se ha pulsado el botón
    $date = date('Y-m-d');
    $filename = "CargasEnAduana_$date.xls";
