...
$query = "SELECT carga.booking, cntr.cntr_number, cntr.cntr_type, carga.final_point, cntr.status_cntr  FROM cntr INNER JOIN carga ON carga.booking = cntr.booking WHERE main_status = 'ON BOARD' AND `user_cntr` = '$user'";
...
if(isset($_POST["export_data"])) { //  comprobar que se ha pulsado el botón
    $date = date('Y-m-d');
    $filename = "CargasEmbarcadas_$date.xls";
