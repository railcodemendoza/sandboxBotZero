...
$query = "SELECT `booking`,`cntr_number`,`cntr_type`,`main_status`,`retiro_place` FROM `cntr` WHERE main_status = 'YENDO A CARGAR' AND `user_cntr` = '$user'";
...
if(isset($_POST["export_data"])) { //  comprobar que se ha pulsado el botón
    $date = date('Y-m-d');
    $filename = "CargasCaminoACargar_$date.xls";
