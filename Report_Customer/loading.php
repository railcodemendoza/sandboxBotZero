...
$query = "SELECT carga.booking, cntr.cntr_number, cntr.cntr_type, cntr.status_cntr, carga.load_place FROM cntr INNER JOIN carga ON carga.booking = cntr.booking WHERE main_status = 'CARGANDO' AND `user_cntr` = '$user'";
...
if(isset($_POST["export_data"])) { //  comprobar que se ha pulsado el botón
    $date = date('Y-m-d');
    $filename = "CargasCargando_$date.xls";
