...
$query = "SELECT carga.booking, cntr.cntr_number, cntr.cntr_type, cntr.status_cntr, carga.unload_place, carga.cut_off_fis FROM cntr INNER JOIN carga ON carga.booking = cntr.booking WHERE main_status = 'YENDO A DESCARGAR' AND `user_cntr` = '$user'";
...
if(isset($_POST["export_data"])) { //  comprobar que se ha pulsado el botón
    $date = date('Y-m-d');
    $filename = "CargasCaminoDescargar_$date.xls";
