...
$query = "SELECT id, ref_customer, booking, shipper, commodity, load_place, load_date, unload_place, oceans_line, final_point, ETA, ETD, consignee, notify, user FROM `carga` WHERE empresa = '$company'  AND status = 'TERMINADA'";
...
if(isset($_POST["export_data"])) { //  comprobar que se ha pulsado el botón
    $date = date('Y-m-d');
    $filename = "CargasTerminasdas_$date.xls";
