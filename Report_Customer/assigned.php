...
$query = "SELECT carga.booking, cntr.cntr_number, cntr.cntr_type, asign.transport, asign.transport_agent, asign.driver, asign.truck, asign.truck_semi, cntr.status_cntr 
        FROM cntr 
        INNER JOIN carga 
        INNER JOIN asign 
        ON carga.booking = cntr.booking 
        AND cntr.booking = asign.booking 
        WHERE cntr.main_status = 'ASIGNADA' 
        AND `user_cntr` = '$user'";
...
if(isset($_POST["export_data"])) { //  comprobar que se ha pulsado el botón
    $date = date('Y-m-d');
    $filename = "CargasConUnidadAsignada_$date.xls";
