...
$user = $_SESSION['user'];
        ...
        $query_all = "SELECT count(main_status) from cntr where `user_cntr` = '$user'";
        $result_all= mysqli_query($conn, $query_all);
        if (mysqli_num_rows($result_all) == 1) {
            $row = mysqli_fetch_array($result_all);
            $cantidad_all =  $row['count(main_status)'];}  
            
        $query_detalles_go_to_load = "SELECT cntr_number , cntr_type, retiro_place, status_cntr from cntr where `user_cntr` = '$user' AND `main_status` = 'YENDO A CARGAR'";
        $result_detalles_go_to_load= mysqli_query($conn, $query_detalles_go_to_load);

        $query_detalles_loading = "SELECT cntr.cntr_number , cntr.cntr_type, carga.load_place, cntr.status_cntr from cntr INNER JOIN carga ON carga.booking = cntr.booking where  cntr.user_cntr = '$user' AND cntr.main_status = 'CARGANDO'";
        $result_detalles_loading= mysqli_query($conn, $query_detalles_loading);
        
        $query_detalles_custom_place = "SELECT cntr.cntr_number , cntr.cntr_type, carga.custom_place, cntr.status_cntr from cntr INNER JOIN carga ON carga.booking = cntr.booking where  cntr.user_cntr = '$user' AND cntr.main_status = 'CUSTOM PLACE'";
        $result_detalles_custom_place= mysqli_query($conn, $query_detalles_custom_place);
        
        $query_detalles_go_to_unload = "SELECT cntr.cntr_number , cntr.cntr_type, carga.unload_place, cntr.status_cntr from cntr INNER JOIN carga ON carga.booking = cntr.booking where  cntr.user_cntr = '$user' AND cntr.main_status = 'YENDO A DESCARGAR'";
        $result_detalles_go_to_unload= mysqli_query($conn, $query_detalles_go_to_unload);

        $query_detalles_staking = "SELECT cntr.cntr_number , cntr.cntr_type, carga.unload_place, cntr.status_cntr from cntr INNER JOIN carga ON carga.booking = cntr.booking where  cntr.user_cntr = '$user' AND cntr.main_status = 'STAKING'";
        $result_detalles_staking= mysqli_query($conn, $query_detalles_staking);

        $query_detalles_on_board = "SELECT cntr.cntr_number , cntr.cntr_type, carga.vessel, carga.voyage, cntr.status_cntr from cntr INNER JOIN carga ON carga.booking = cntr.booking where  cntr.user_cntr = '$user' AND cntr.main_status = 'ON BOARD'";
        $result_detalles_on_board= mysqli_query($conn, $query_detalles_on_board);

        $query_detalles_all = "SELECT cntr.cntr_number , cntr.cntr_type, carga.shipper, cntr.status_cntr from cntr INNER JOIN carga ON carga.booking = cntr.booking where `user_cntr` = '$user'";
        $result_detalles_all= mysqli_query($conn, $query_detalles_all);

        $query_detalles_asigned = "SELECT cntr_number , cntr_type, retiro_place, status_cntr from cntr where `user_cntr` = '$user' AND `main_status` = 'ASIGNADA'";
...
<div style="background:linear-gradient(to left, #17A589, #2E303E );" class="breadcrumbs">
    <div style="background:linear-gradient(to left, #17A589, #2E303E );" class="breadcrumbs-inner">  
        <div class="row">
            <div class="col-lg-3 col-md-6">
                <div  style="box-shadow: 4px 1px 18px 3px #1BA5B4" class="card">
                    <div class="card-body">
                        ...
                        <div data-toggle="modal" data-target="#ASIGNADA" class="stat-icon dib flat-color-3">
                            ...
                            <div class="stat-content">
                                <div class="text-left dib">
                                    ...
                                    <div class="stat-heading">ASIGNADA</div>
                                </div>
                            </div>
                        </div>
                    </div>
